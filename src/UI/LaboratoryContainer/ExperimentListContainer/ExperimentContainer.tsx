...
import {ExperimentContainerProps} from "../LaboratoryContainer.types";
...
export default function ExperimentsContainerComponent({onExperimentClick, loadDataOnInit = true, mode, queryData, experimentSelected, onExperimentSelect }: ExperimentContainerProps) {
  ...
  const [sharingExperiment, setSharingExperiment] = useState(false);
    ...
    const onRestoreExperiment = (experiment: Experiment) => {
      const { id } = experiment || {};
  
      alertify.notify("Restoring experiment...", "info");
    
      restoreExperiment(id).then((response) => {
        if (response.errorCode) {
          alertify.error("Error when trying to restore the experiment");
        } else {
          alertify.dismissAll();
